...
import qualified JobHandler.Memory as Memory
...
cleanupDocker = void do
  Process.readProcessStdout "~/.rd/bin/docker rm -f $(~/.rd/bin/docker ps -aq --filter \"label=quad\")"
  Process.readProcessStdout "~/.rd/bin/docker volume rm -f $(~/.rd/bin/docker volume ls -q --filter \"label=quad\")"
...
main = hspec do
  ...
  beforeAll cleanupDocker $
    describe "Quad CI" do
      ...
      it "shouldo process webhooks" do
...
testImagePull runner = do
  Process.readProcessStdout "~/.rd/bin/docker rmi -f busybox"
...
testWebhookTrigger =
  runServerAndAgent $ \handler -> do
    ...
    let Right (Aeson.Object build) = Aeson.eitherDecodeStrict $ HTTP.getResponseBody response
